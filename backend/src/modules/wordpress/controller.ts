...
import { isTaskRunning, requestCancelTask, startImportTask } from './importQueue';
...
const CIPHER_KEY = process.env.CIPHER_KEY ?? 'freshcart_wp_key_2025';
...
export class WordPressController {
  ...
  previewProducts = asyncHandler(async (req: Request, res: Response) => {
    ...
    const products = await wordPressService.fetchProducts(setting, page, perPage);
    const total = await wordPressService.getTotalProductCount(setting);
