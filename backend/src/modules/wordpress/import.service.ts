...
import { WCProduct, WCSetting, WCVariation, wordPressService } from './wordpress.service';
  ...
  if (extraImages.length > 0) {
    ...
    for (const img of extraImages) {
      try {
        ...
        galleryImages.push(`${config.apiUrl}${saved}`);
      } catch (e: any) { 
