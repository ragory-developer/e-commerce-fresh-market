...
import { importCategories, importProduct } from './import.service';
import { WCSetting, wordPressService } from './wordpress.service';
...
async function runTask(
  ...
  const logFn = (msg: string) => {
    ...
    if (details.length > 500) details.shift();
  ...
  try {
    ...
    if (productIds && productIds.length > 0) {
      ...
      for (const pid of productIds) {
        ...
        try {
          const url = `${setting.siteUrl.replace(/\/$/, '')}/wp-json/${setting.apiVersion ?? 'wc/v3'}/products/${pid}`;
          ...
          const resp = await fetch(url, {
            headers: {
              Authorization:
                'Basic ' +
                Buffer.from(`${setting.consumerKey}:${setting.consumerSecret}`).toString('base64'),
            },
            signal: controller.signal,
          });
          clearTimeout(tId);
          if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
          wcProducts.push(await resp.json());
        } catch (err: any) {
          failed++;
