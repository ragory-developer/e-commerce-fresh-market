  ...
  static async adjustGlobalBalance(amount: number, type: 'TOPUP' | 'DEDUCTION' | 'REFUND', note: string, userId?: string, tx?: any) {
    try {
      const executeOperation = async (prismaClient: any) => {
        // 1. Fetch current global balance
        const balanceSetting = await prismaClient.setting.findUnique({ where: { key: 'wallet_balance' } });
        ...
        // 5. Enforce 100 records limit (Rolling History) - Run in background or separate to not block main operation
        try {
          const count = await prismaClient.walletTransaction.count();
          if (count > 100) {
            const oldestRecords = await prismaClient.walletTransaction.findMany({
              orderBy: { createdAt: 'asc' },
              take: count - 100,
              select: { id: true }
            });
            await prismaClient.walletTransaction.deleteMany({
              where: { id: { in: oldestRecords.map((r: any) => r.id) } }
            });
          }
        } catch (cleanupError: any) {
          console.error(`[Wallet] History cleanup failed: ${cleanupError.message}`);
          // Non-blocking: we continue even if cleanup fails
        }
